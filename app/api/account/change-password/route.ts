...
export async function POST(request:Request){
  ...
  if(password!==confirmPassword||password.length<10||!/[A-Z]/.test(password)||!/[a-z]/.test(password)||!/[0-9]/.test(password))return NextResponse.json({error:"Use matching passwords with 10+ characters, uppercase, lowercase, and a number."},{status:400});
  ...
  await supabaseFetch("/rest/v1/notifications",{method:"POST",headers:{Prefer:"return=minimal"},body:JSON.stringify({user_id:session.user.id,title:"Password changed",body:"Your LUMA account password was changed successfully."})},undefined,true);
